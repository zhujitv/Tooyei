...
const recipients = async () => {
  const envRecipients = process.env.INQUIRY_NOTIFICATION_EMAIL
    ?.split(",")
    .map((value) => value.trim())
    .filter(Boolean) ?? [];
  if (envRecipients.length > 0) return envRecipients;
  const settings = await getAdminSiteSettings();
  return [settings.inquiryEmail, settings.notificationEmail].filter((value, index, values) => value && values.indexOf(value) === index);
};

const fromAddress = () => process.env.RESEND_FROM_EMAIL || "Tooyei <onboarding@resend.dev>";

const siteOrigin = async () =>
  (process.env.NEXT_PUBLIC_SITE_URL || (await getAdminSiteSettings()).siteUrl || "https://tooyei.vercel.app").replace(/\/$/, "");
...
const buildHtml = (inquiry: AdminInquiryDetail, origin: string) => {
  const adminUrl = `${origin}/admin/inquiries/${inquiry.id}`;
...
export async function sendInquiryNotification(inquiry: AdminInquiryDetail): Promise<SendInquiryNotificationResult> {
  ...
  const [to, origin] = await Promise.all([recipients(), siteOrigin()]);
