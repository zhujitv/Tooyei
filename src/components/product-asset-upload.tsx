...
export function ProductAssetUpload({ action, disabled = false }: Props) {
  return (
    <form action={action} className="space-y-4 rounded-xl border border-white/10 bg-white/[0.04] p-4">
      <div className="flex items-center gap-2">
        <FileUp className="size-5 text-[#d6b36a]" />
        <div>
          <h3 className="font-medium text-white">上传图片、视频或产品资料</h3>
          <p className="text-xs text-white/40">文件上传到 Vercel Blob，完成后自动关联当前产品；单个文件最大 3.8 MB。</p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-white/50">图片/视频请选择“产品媒体”，PDF、Word、Excel 请选择“下载资料”。</p>
        <Button type="submit" disabled={disabled} className="bg-[#b68a4c] text-[#0b1220] hover:bg-[#c59b5c]">
          <Upload />
          上传并关联
        </Button>
      </div>
