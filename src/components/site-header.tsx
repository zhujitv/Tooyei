...
> = {
  ...
  fr: {
    ...
    nav: { products: "Produits", applications: "Applications", oem: "OEM / ODM", support: "Aide à l’achat", about: "Entreprise", contact: "Contact" },
  },
  es: {
  ...
  ru: {
    ...
    nav: { products: "Продукция", applications: "Применение", oem: "OEM / ODM", support: "Поддержка закупок", about: "О компании", contact: "Контакты" },
  },
  ja: {
    ...
    nav: { products: "製品", applications: "用途", oem: "OEM / ODM", support: "調達サポート", about: "会社情報", contact: "お問い合わせ" },
  },
  it: {
    ...
    nav: { products: "Prodotti", applications: "Applicazioni", oem: "OEM / ODM", support: "Supporto acquisti", about: "Azienda", contact: "Contatti" },
  },
  ar: {
    ...
    nav: { products: "المنتجات", applications: "التطبيقات", oem: "OEM / ODM", support: "دعم التوريد", about: "عن الشركة", contact: "اتصل بنا" },
  },
};
...
export function SiteHeader({ locale, initialCategories = [] }: { locale: Locale; initialCategories?: PublicCategoryNode[] }) {
  ...
  const desktopNavLink = (item: (typeof sectionItems)[number]) => {
    const active = (item.key === "contact" && neutralPath.startsWith("/contact")) || (neutralPath === "/" && item.section === activeSection);
    return (
