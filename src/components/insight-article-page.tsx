...
export async function InsightArticlePage({ locale, slug }: { locale: Locale; slug: string }) {
  ...
  const breadcrumbJsonLd = buildArticleBreadcrumbJsonLd({
    ...
    localePath: localizedPath(locale, "/insights"),
    insightsLabel: labels.eyebrow,
    category: { path: localizedPath(locale, `/insights/category/${article.category.slug}`), label: article.category.name },
    title: article.title,
  });
  const related = articles
    .filter((item) => item.id !== article.id)
    .sort((left, right) => Number(right.category.id === article.category.id) - Number(left.category.id === article.category.id))
    .slice(0, 3);
  const categoryPath = localizedPath(locale, `/insights/category/${article.category.slug}`);
